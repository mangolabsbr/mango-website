...
- To write a new article, follow `content/articles/README.md` (authoring guide: file layout, frontmatter rules, MDX patterns, checklist). One article = `content/articles/<slug>/{en,es,pt}.mdx`. Frontmatter: `title`, `description`, quoted `date` ("YYYY-MM-DD"), and `thumbnail` (a `/public` path, 1200×630). All three locales must exist; `lib/articles.ts` falls back to `en` if one is missing.
